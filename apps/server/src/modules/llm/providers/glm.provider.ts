...
/**
 * GLMProvider is a placeholder implementation of the LLMProvider contract.
 * It validates that GLM_API_KEY is present at boot time. The body of
...
export class GLMProvider implements LLMProvider {
  ...
  constructor(config: ConfigService) {
    const key = config.get<string>('GLM_API_KEY');
    if (!key || key.trim().length === 0) {
      throw new LLMError(
        LLMErrorCode.SERVICE_ERROR,
        this.name,
        'GLM_API_KEY is missing. Set it in the environment before starting the server.',
      );
    }
    this.apiKey = key;
  }
